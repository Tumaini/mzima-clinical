    ...
    implements DataServiceManagerAccess, LiveDataService
{
    ...
    public com.mcddb.data.Sideeffects getSideeffectsById(Integer id, PagingOptions pagingOptions) {
        List<com.mcddb.data.Sideeffects> rtn = ((List<com.mcddb.data.Sideeffects> ) dsMgr.invoke(taskMgr.getQueryTask(), (McdDBConstants.getSideeffectsByIdQueryName), id, pagingOptions));
        if (rtn.isEmpty()) {
            return null;
        } else {
            return rtn.get(0);
